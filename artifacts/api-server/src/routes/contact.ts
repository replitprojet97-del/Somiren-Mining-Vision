...
/** Escape all HTML special chars to prevent injection */
...
router.post("/contact", async (req, res) => {
  ...
  const { error } = await resend.emails.send({
    ...
    to: [toEmail],
    replyTo: email,
    subject: `[Contact Somiren] ${safeSubject}`,
    html: `<!DOCTYPE html>
<html lang="fr">
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;margin:0 auto;background:#0d0d0d;border-top:3px solid #B8952A;">
        ...
        <tr>
          <td style="padding:28px 40px 0;">
            ...
            <h1 style="margin:8px 0 0;font-size:22px;color:#ffffff;">${safeSubject}</h1>
          </td>
        </tr>
        <tr>
          <td style="padding:24px 28px;">
            <table width="100%" cellpadding="0" cellspacing="0" style="background:#1a1a1a;border-radius:4px;">
              <tr style="border-bottom:1px solid #2a2a2a;">
                ...
                <td style="padding:6px 12px;font-size:13px;color:#fff;">${safeName}</td>
              </tr>
              <tr style="border-bottom:1px solid #2a2a2a;">
                <td style="padding:6px 12px;color:#999;font-size:13px;">Email</td>
                <td style="padding:6px 12px;font-size:13px;"><a href="mailto:${safeEmail}" style="color:#B8952A;">${safeEmail}</a></td>
              </tr>
              ${phoneRow}
            </table>
          </td>
        ...
        <tr>
          <td style="padding:0 28px 32px;">
            <table width="100%" cellpadding="0" cellspacing="0" style="background:#1a1a1a;border-radius:4px;border-left:3px solid #B8952A;">
              <tr>
                <td style="padding:20px;font-size:14px;line-height:1.7;color:#ccc;white-space:pre-wrap;">${safeMessage}</td>
              </tr>
            </table>
          </td>
        </tr>
        <tr>
          <td style="padding:16px 40px;background:#080808;border-top:1px solid #222;">
            <p style="margin:0;font-size:11px;color:#555;">Envoyé depuis <a href="https://somiren.com" style="color:#B8952A;">somiren.com</a></p>
          </td>
        </tr>
      </table>
    </td></tr>
